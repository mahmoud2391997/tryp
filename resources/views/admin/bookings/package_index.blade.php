...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-gray-800">Package Bookings</h1>
    ...
    <div class="mb-6">
        <div class="border-b border-gray-200">
            <nav class="-mb-px flex space-x-8">
                <a href="{{ route('admin.bookings.index', ['type' => 'all']) }}" class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300">
                    All Bookings
                </a>
                <a href="{{ route('admin.bookings.index', ['type' => 'bundles']) }}" class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300">
                    Bundle Bookings
                </a>
                <a href="{{ route('admin.bookings.index', ['type' => 'packages']) }}" class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm text-blue-600">
                    Package Bookings
                </a>
            </nav>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6 mb-8">
        <form method="GET" action="{{ route('admin.bookings.index', ['type' => 'packages']) }}" class="flex flex-col md:flex-row gap-4">
            <div class="flex-1">
            ...
            <div class="w-full md:w-48">
                <label for="package_type" class="sr-only">Package Type</label>
                <select id="package_type" name="package_type" class="block w-full py-2 pl-3 pr-10 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md">
                    <option value="">All Packages</option>
                    @foreach($packages as $package)
                        <option value="{{ $package->type }}" {{ request('package_type') == $package->type ? 'selected' : '' }}>{{ $package->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <a href="{{ route('admin.bookings.index', ['type' => 'packages']) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                Reset
            </a>
        </form>
    </div>
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <div class="text-xs text-gray-500">
                                Type: {{ ucfirst($booking->package_type) }}
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex justify-end space-x-3">
                                <a href="{{ route('admin.bookings.show', ['booking' => $booking->id, 'type' => 'package']) }}" class="text-indigo-600 hover:text-indigo-900">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                    </svg>
                                </a>
                                <a href="{{ route('admin.bookings.edit', ['booking' => $booking->id, 'type' => 'package']) }}" class="text-blue-600 hover:text-blue-900">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
