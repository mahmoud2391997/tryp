...
@section('content')
    <div class="container px-4 mx-auto">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6">
            <h1 class="text-2xl font-semibold text-gray-800">Add Payment Gateway</h1>
                ...
                <form action="{{ route('admin.payment-gateways.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div>
                            ...
                            <select class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('gateway_type') border-red-300 text-red-900 focus:outline-none focus:ring-red-500 focus:border-red-500 @enderror" 
                                ...
                                @foreach($gatewayTypes as $value => $label)
                                    <option value="{{ $value }}" {{ old('gateway_type') == $value ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @error('gateway_type')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <div class="mt-1 flex items-center">
                                ...
                                <input type="file" id="icon" name="icon" class="ml-5 bg-white py-2 px-3 border border-gray-300 rounded-md shadow-sm text-sm leading-4 font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 @error('icon') border-red-300 text-red-900 @enderror">
                            </div>
                            <p class="mt-1 text-sm text-gray-500">Upload an icon image for this payment method.</p>
                            @error('icon')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div>
                            <div class="relative flex items-start">
                                <div class="flex items-center h-5">
                                    <input type="checkbox" id="is_active" name="is_active" {{ old('is_active') ? 'checked' : '' }} class="focus:ring-blue-500 h-4 w-4 text-blue-600 border-gray-300 rounded">
                                </div>
                        ...
                        <div>
                            <div class="relative flex items-start">
                                <div class="flex items-center h-5">
                                    <input type="checkbox" id="is_default" name="is_default" {{ old('is_default') ? 'checked' : '' }} class="focus:ring-blue-500 h-4 w-4 text-blue-600 border-gray-300 rounded">
                                </div>
                                <div class="ml-3 text-sm">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                <div>
                                    ...
                                    <input type="text" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" 
                                        id="stripe_api_key" name="stripe_api_key" value="{{ old('stripe_api_key') }}">
                                </div>
                                <div>
                                    ...
                                    <input type="password" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" 
                                        id="stripe_secret_key" name="stripe_secret_key" value="{{ old('stripe_secret_key') }}">
                                </div>
                            </div>
                            <div class="mb-6">
                                <label for="stripe_webhook_secret" class="block text-sm font-medium text-gray-700">Webhook Secret</label>
                                <input type="password" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" 
                                    id="stripe_webhook_secret" name="stripe_webhook_secret" value="{{ old('stripe_webhook_secret') }}">
                            </div>
                        </div>
                        ...
                        <div class="gateway-config" id="paypal-config" style="display: none;">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                <div>
                                    ...
                                    <input type="text" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" 
                                        id="paypal_client_id" name="paypal_client_id" value="{{ old('paypal_client_id') }}">
                                </div>
                                <div>
                                    <label for="paypal_secret" class="block text-sm font-medium text-gray-700">Secret</label>
                                    <input type="password" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" 
                                        id="paypal_secret" name="paypal_secret" value="{{ old('paypal_secret') }}">
                                </div>
                            </div>
                            <div class="mb-6">
                                ...
                                <select class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" 
                                    ...
                                    <option value="sandbox" {{ old('paypal_environment') == 'sandbox' ? 'selected' : '' }}>Sandbox (Testing)</option>
                                    <option value="production" {{ old('paypal_environment') == 'production' ? 'selected' : '' }}>Production (Live)</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="gateway-config" id="authorize_net-config" style="display: none;">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                <div>
                                    ...
                                    <input type="text" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" 
                                        id="authorize_login_id" name="authorize_login_id" value="{{ old('authorize_login_id') }}">
                                </div>
                                <div>
                                    <label for="authorize_transaction_key" class="block text-sm font-medium text-gray-700">Transaction Key</label>
                                    <input type="password" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" 
                                        id="authorize_transaction_key" name="authorize_transaction_key" value="{{ old('authorize_transaction_key') }}">
                                </div>
                            </div>
                            <div class="mb-6">
                                <div class="relative flex items-start">
                                    <div class="flex items-center h-5">
                                        <input type="checkbox" id="authorize_sandbox_mode" name="authorize_sandbox_mode" {{ old('authorize_sandbox_mode') ? 'checked' : '' }} class="focus:ring-blue-500 h-4 w-4 text-blue-600 border-gray-300 rounded">
                                    </div>
                                    <div class="ml-3 text-sm">
